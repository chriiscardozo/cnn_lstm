...
def tf_shrelu(alpha, x):
	x_2 = tf.square(x)
	t_2 = tf.square(alpha)
	numerador = x + tf.sqrt(x_2 + t_2)
	return numerador / tf.constant(2.0)

# ****************************************************************
# Código desenvolvido no trabalho da aula 01 para MNIST com CNN
# Mudança: uso da função shrelu com parâmetros no aprendizado
# ****************************************************************
from tensorflow.examples.tutorials.mnist import input_data
mnist = input_data.read_data_sets("MNIST_data/", one_hot=True)
...
alpha2 = alpha_variable([1, 14, 14, 64])
...
h_conv1 = tf_shrelu(alpha1, conv2d(x_image, W_conv1) + b_conv1)
...
h_conv2 = tf_shrelu(alpha2, conv2d(h_pool1, W_conv2) + b_conv2)
...
with tf.Session() as sess:
	...
	for i in range(20000):
		...
		if i % 100 == 0:
			...
			if(1-test_accuracy <= tolerancia): break
